...
# Start to generate krelpin_command file
COMMAND_FILE="$OUTPUT/krelpin_command"
echo '# Generated for Krelpin' > "$COMMAND_FILE"
...
elif [ -f "$HERE/$file" ]; then
    print_info "File ("$HERE/$file") found in device directory, copying."
...
XZ_OPT="-T0" tar xpf "../$file" --numeric-owner -C system
...
echo "update $file $file.asc" >> "$COMMAND_FILE"
...
# End krelpin_command
echo 'unmount system' >> "$COMMAND_FILE"
ln -sf krelpin_command "$OUTPUT/ubuntu_command"
